import type { ActiveMessage } from "../../lib/active-message";
    ...
    <article className="overflow-hidden rounded-2xl border border-hair bg-surface">
      <div className="flex gap-4 px-8 py-6">
        <span
          ...
          style={{ backgroundColor: personaColor(message.personaColor) }}
          aria-hidden
        />
        <div className="flex flex-1 flex-col gap-4">
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-3">
              <PersonaAvatar name={message.personaName} color={message.personaColor} size={56} />
              <div className="flex flex-col">
                <span className="text-[15px] font-medium text-ink-primary">
                  {message.personaName}
                </span>
                <span className="text-[13px] text-ink-muted">{message.personaDemographic}</span>
              </div>
            </div>
